...
pSampler::pSampler(const std::string &dense_folder, int ncams) {
  ...
  // try to load the final image in the folders to assess if we are running this
  // with priors.
  std::stringstream norm_path;
  std::stringstream depth_path;
  norm_path << normal_folder << "/" << std::setw(8) << std::setfill('0')
            << (ncams - 1) << ".png";
  depth_path << depth_folder << "/" << std::setw(8) << std::setfill('0')
             << (ncams - 1) << ".png";
...
void get3DPoint(const Camera camera, const int2 p, const float depth,
                float *X) {
...
float4 depth_normal_to_plane(const float depth, float3 normal, int2 p,
                             Camera camera) {
  ...
  float dot_product = temp.x * view_direction.x + temp.y * view_direction.y +
                      temp.z * view_direction.z;
  if (dot_product > 0.0f) {
    temp.x = -temp.x;
    temp.y = -temp.y;
    temp.z = -temp.z;
...
bool pSampler::confirm_using_prior() const { return using_prior; }

float3 local_v3tof3(cv::Vec3f inp) {
  return make_float3(inp[0], inp[1], inp[2]);
}

std::unique_ptr<float4> pSampler::GetPriorPlaneEstimate(const int camNum,
                                                        Camera cam,
                                                        const int rows,
                                                        const int cols) {
  using cv::Mat;
  ...
  Mat conv_normals;
  normals.convertTo(conv_normals, CV_32FC3, 2.0 / 65536.0, -1.0);
  const float dist = (cam.depth_max - cam.depth_min);
  const float range = dist / 65535.0f;
  Mat conv_depths;
  depth.convertTo(conv_depths, CV_32F, range, cam.depth_min
                  // 0
  );
  ...
  for (int i = 0; i < rows; i++) {
    for (int j = 0; j < cols; ++j) {
      ...
      float4 plane = depth_normal_to_plane(
          base_d,
          local_v3tof3(conv_normals.at<cv::Vec3f>(i * scale, j * scale)),
          make_int2(j, i), cam);
      //            if((i%100 == 0) && (j%100 ==0)){
      //                std::cout << "loaded depth value: " << base_d <<
      //                std::endl;
      //            }
      //           plane = make_float4(0,0,0,0);
      plane_hypothesis[k] = plane;
      depths_im(i, j) = plane.w;
      normals_im(i, j) = cv::Vec3f(plane.x, plane.y, plane.z);
    }
  }
#ifdef DEBUG
  Mat depths_vis;
...
void ProcessProblem(pSampler &pSample, const std::string output_folder,
  ...
  ACMMP acmmp; // here we initialise an acmmp handler: it's a single instance
               // object
  if (geom_consistency) {
  ...
  if (planar_prior) {
      ...
      if (imageRC.contains(triangle.pt1) && imageRC.contains(triangle.pt2) &&
        ...
        for (float p = 0; p < 1.0; p += step) {
          for (float q = 0; q < 1.0 - p; q += step) {
            ...
            mask_tri(y, x) =
                idx +
                1.0; // To distinguish from the label of non-triangulated areas
          }
        }
    ...
    for (int col = 0; col < width; ++col) {
      for (int row = 0; row < height; ++row) {
        ...
        float4 plane_hypothesis = acmmp.GetPlaneHypothesis(center);
        depths(row, col) = plane_hypothesis.w;
        normals(row, col) = cv::Vec3f(plane_hypothesis.x, plane_hypothesis.y,
                                      plane_hypothesis.z);
        costs(row, col) = acmmp.GetCost(center);
      }
    }
...
void JointBilateralUpsampling(const std::string &dense_folder,
  ...
  std::cout << "Run JBU for image " << problem.ref_image_id << ".jpg"
            << std::endl;
  RunJBU(scaled_image_float, ref_depth, dense_folder, output_folder, problem);
}
...
c_info get_consistency_metrics(const std::vector<Camera> &cameras, size_t i,
                               int r, int c, float ref_depth,
                               const cv::Vec3f &ref_normal, int src_id,
                               int src_r, int src_c, float src_depth_0,
                               const cv::Vec3f &src_normal_0, float src_depth_1,
                               const cv::Vec3f &src_normal_1) {
  // this component needs to handle the depth checking results
  cmetric res0;
  if (src_depth_0 > 0) {
    float3 tmp_X =
        Get3DPointonWorld(src_c, src_r, src_depth_0, cameras[src_id]);
    float2 tmp_pt;
  ...
  if (src_depth_1 > 0) {
    float3 tmp_X =
        Get3DPointonWorld(src_c, src_r, src_depth_1, cameras[src_id]);
    float2 tmp_pt;
    float proj_depth;
    ProjectonCamera(tmp_X, cameras[i], tmp_pt, proj_depth);
  ...
  result.y = src_r;
  float dc0 = exp(
      -(res0.reproj_err + 200 * res0.relative_depth_diff + res0.angle * 10));
  float dc1 = exp(
      -(res1.reproj_err + 200 * res1.relative_depth_diff + res1.angle * 10));
...
void getCandidates(const std::vector<Problem> &problems,
                   ...
                   std::map<int, int> &image_id_2_index, size_t i, int num_ngb,
                   int r, int c, float ref_depth, const cv::Vec3f &ref_normal) {
  ...
  for (int j = 0; j < num_ngb; ++j) { // For every other pair cam, for the
    ...
    if (src_c >= 0 && src_c < src_cols && src_r >= 0 && src_r < src_rows) {
      ...
      float src_p_depth = prior_depths[src_id].at<float>(src_r, src_c);
      cv::Vec3f src_p_normal =
          prior_normals[src_id].at<cv::Vec3f>(src_r, src_c);

      auto metric = get_consistency_metrics(
          cameras, i, r, c, ref_depth, ref_normal, src_id, src_r, src_c,
          src_depth, src_normal, src_p_depth, src_p_normal);
      consistency_candidates.push_back(metric);
    }
  }
}

void RunPriorAwareFusion(std::string &dense_folder, std::string &outfolder,
                         std::string &fusion_folder,
                         const std::vector<Problem> &problems,
                         bool geom_consistency, float consistency_scalar,
                         int num_consistent_thresh, int single_match_penalty,
  ...
  for (size_t i = 0; i < num_images; ++i) {
    ...
    std::stringstream result_path;
    result_path << fusion_folder << "/2333_" << std::setw(8)
                << std::setfill('0') << problems[i].ref_image_id;
    std::string result_folder = result_path.str();
    std::string suffix = "/depths.dmb";
    if (geom_consistency) {
      suffix = "/depths_geom.dmb";
    ...
    prior_result_path << outfolder << "/2333_" << std::setw(8)
                      << std::setfill('0') << problems[i].ref_image_id;
    ...
    if (!(mask_folder == " ")) {
      ...
      mask_path << mask_folder << "/" << std::setw(8) << std::setfill('0')
                << problems[i].ref_image_id << ".png";
      cv::Mat_<cv::Vec3b> mask_image = cv::imread(mask_path.str(), -1);
    ...
    for (int r = 0; r < rows; ++r) {
      for (int c = 0; c < cols; ++c) {
        ...
        if (ref_depth > 0.0) { // for every camera get the rdepth proj
          // do the calculation
          getCandidates(problems, cameras, depths, prior_depths, normals,
                        prior_normals, masks, consistency_candidates_0,
                        image_id_2_index, i, num_ngb, r, c, ref_depth,
                        ref_normal);
          ...
          d_thresh_0 = (n_consistent_0 >= num_consistent_thresh) &&
                       (d_cons_0 > consistency_scalar * n_consistent_0);
        }
        ...
        if (ref_p_depth > 0.0) {
          ...
          getCandidates(problems, cameras, depths, prior_depths, normals,
                        prior_normals, masks, consistency_candidates_1,
                        image_id_2_index, i, num_ngb, r, c, ref_p_depth,
                        ref_p_normal);
          // use the data
        ...
        // if one passes, make them take a harsher test.
          ...
          d_thresh_1 = d_thresh_1 && (n_consistent_1 >= (num_consistent_thresh +
                                                         single_match_penalty));
          passing = d_thresh_1;
          to_iterate = consistency_candidates_1;
          g_depth = ref_p_depth;
          g_normal = ref_p_normal;
        } else {
          d_thresh_0 = d_thresh_0 && (n_consistent_0 >= (num_consistent_thresh +
                                                         single_match_penalty));
          passing = d_thresh_0;
          to_iterate = consistency_candidates_0;
          g_depth = ref_depth;
        ...
        if (passing) {
          ...
          float3 PointX = Get3DPointonWorld(c, r, g_depth, cameras[i]);
          float3 consistent_Point = PointX;
          cv::Vec3f consistent_normal = g_normal;
          float consistent_Color[3] = {(float)images[i].at<cv::Vec3b>(r, c)[0],
          ...
          point3D.color = make_float3(consistent_Color[0], consistent_Color[1],
                                      consistent_Color[2]);
...
void RunFusion(std::string &dense_folder, std::string &outfolder,
               ...
               const std::string &image_dir, const std::string &mask_folder) {
  size_t num_images = problems.size();
  ...
  for (size_t i = 0; i < num_images; ++i) {
    ...
    if (!(mask_folder == " ")) {
      std::stringstream mask_path;
      mask_path << dense_folder << "/" << mask_folder << "/" << std::setw(8)
                << std::setfill('0') << problems[i].ref_image_id << ".png";
      //            std::cout << mask_path.str() << std::endl;
      cv::Mat mask_image = cv::imread(mask_path.str(), -1);
  ...
  for (size_t i = 0; i < num_images; ++i) {
      ...
      for (int c = 0; c < cols; ++c) {
        ...
        for (int j = 0; j < num_ngb; ++j) {
          ...
          if (src_c >= 0 && src_c < src_cols && src_r >= 0 &&
            ...
            float3 tmp_X =
                Get3DPointonWorld(src_c, src_r, src_depth, cameras[src_id]);
            float2 tmp_pt;
            ProjectonCamera(tmp_X, cameras[i], tmp_pt, proj_depth);
            float reproj_error =
            ...
            if (reproj_error < 2.0f && relative_depth_diff < 0.01f &&
              ...
              consistent_Color[0] += images[src_id].at<cv::Vec3b>(src_r,
              src_c)[0]; consistent_Color[1] +=
              images[src_id].at<cv::Vec3b>(src_r, src_c)[1]; consistent_Color[2]
              += images[src_id].at<cv::Vec3b>(src_r, src_c)[2];*/
              used_list[j].x = src_c;
              used_list[j].y = src_r;

              float tmp_index =
                  reproj_error + 200 * relative_depth_diff + angle * 10;
        ...
        if (num_consistent >= con_num_thresh &&
            (dynamic_consistency > consistency_scalar * num_consistent)) {
          num_approved += 1;
          ...
          for (int j = 0; j < num_ngb; ++j) {
            if (used_list[j].x == -1)
              continue;
            masks[image_id_2_index[problems[i].src_image_ids[j]]].at<uchar>(
    ...
    if (fuck_counter > 0)
      std::cout << "found " << fuck_counter << " fucks \n";
