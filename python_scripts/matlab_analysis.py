...
def analyse_path(recon: Path) -> None:
    ...
    if not os.path.exists(outname0):
    ...
    if not os.path.exists(outname0) and os.path.exists(obj_loc0):
    ...
    if not os.path.exists(outname1) and os.path.exists(obj_loc1):
        eng.run_matlab_analysis(camset_num, datapath, obj_loc1, dst, outname1, nargout=0, stdout=io.StringIO())


    obj_loc1 = str(recon/"acmmp_boost_single.ply")
    outname1 = str(recon/"acmmp_boost_single.txt")
    # dtu_eval(
    #     pv.read(ref_loc).points,
    #     pv.read(obj_loc1).points,
    #     outname1,
    ...
    obj_loc1 = str(recon/"ACMMP_full_prior.ply")
    outname1 = str(recon/"ACMMP_full_prior.txt")
    # dtu_eval(
    #     pv.read(ref_loc).points,
    #     pv.read(obj_loc1).points,
    #     outname1,
...
for t in tqdm(tests):
    try:
        ...
        logging.info(f"failed to run {t} with exception:")
        print(str(e))
